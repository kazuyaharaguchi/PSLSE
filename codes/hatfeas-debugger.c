...
int initDist( int *D, int m, int k, int lev );
Boolean getNextDist( int *D, int m, int k );
int searchCombiMain( int *D, int m, int k );
void searchCombi( int *D, int m, int k, int *V, int d, int begin );
...
int main( int argc, char *argv[] ){
  ...
  A1 = (int*)malloc((k-1)*sizeof(int));
  A2 = (int*)malloc((k-1)*sizeof(int));
  while( fscanf( f1, "%s", s1 ) != EOF ){
    ...
    for(t=0;t<k-1;t++)
      A1[t] = atoi( S1[t] );
    f2 = fopen( argv[3], "r" );
    while( fscanf( f2, "%s", s2 ) != EOF ){
      S2 = split( s2, ',' );
      for(t=0;t<k-1;t++)
	A2[t] = atoi( S2[t] );
      if( isEquivHash( A1, A2, m, k ) )
	counter++;
      for(t=0;t<k-1;t++)
	free( S2[t] );
      free( S2 );
    ...
    if( counter != 1 ){
      fprintf( stderr, "%s in 1st file appears %d times in 2nd file\n", 
	       s1, counter );
      exit( EXIT_FAILURE );
    }
    for(t=0;t<k-1;t++)
  ...
  initDist( D, m, k, m-k );
  while( 1 ){
    searchCombiMain( D, m, k );
    if( getNextDist( D, m, k ) == FALSE )
      break;
  }
  return 0;
}
...
void searchCombi( int *D, int m, int k, int *V, int d, int begin ){
  ...
  if( D[d] == 0 )
    return searchCombi( D, m, k, V, d-1, 0 );
  for(l=begin;l<k-1;l++){
    if( V[l] != 0 )
      continue;
    V[l] = d;
    D[d]--;
    searchCombi( D, m, k, V, d, l+1 );
    D[d]++;
    V[l] = 0;
  }
}
